...
def logspace(start, end, steps, *, dtype=None, device=None, layout=None):
    if device is None:
        device = "cuda"
    if dtype is None:
        dtype = torch.float32
    ...
    import math
    log10 = math.log(10)
    logbase_t = torch.full((steps,), log10, dtype=compute_dtype, device=device)

    kernel = _cached_make(ntops.kernels.logspace.premake, output.ndim)
    kernel(start_t, idx_t, step_t, logbase_t, output)
