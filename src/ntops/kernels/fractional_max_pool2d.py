...
def _arrange_output(output, block_size):
    arranged = output.tile((1, 1, 1, 1))
    arranged = arranged.ravel()
    arranged = arranged.flatten(end_dim=4).flatten(start_dim=1)
...
def _arrange_sequence(sequence, input, block_size):
    arranged = sequence.tile((1, 1, 1, 1))
    arranged = arranged.ravel()
    arranged = arranged.flatten(end_dim=4).flatten(start_dim=1)
    arranged = arranged.expand((-1, input.shape[-2] * input.shape[-1]))
    arranged = arranged.tile((block_size, -1))
...
def arrangement(
    ...
    input_arranged = input_arranged.expand(
        (-1, -1, output.shape[-2], output.shape[-1])
    )
...
def application(
    ...
    h_offsets = input.offsets(2)
    w_offsets = input.offsets(3)
...
def application_deterministic(
    ...
    oh_offsets = output.offsets(2)
    ow_offsets = output.offsets(3)
