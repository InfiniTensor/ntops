"""Tune the pinned launch configs for ``ntops.torch.kl_div`` on the current GPU.

Two kernels are tuned independently:
  * the reduction path (``reduction="sum"|"mean"|"batchmean"``) -- the defining,
    perf critical kernel; partials buffer size depends on ``block_size``;
  * the element-wise path (``reduction="none"``).

Performance evaluation runs with auto-tuning disabled (``max_num_configs=1``),
so the values baked into ``ntops/torch/kl_div.py`` decide the score. This script
sweeps a small grid under those exact conditions and prints, per shape, the
fastest config plus the speedup over ``torch.nn.functional.kl_div``. The default
``log_target=False`` path (extra ``log`` + ``where``) is the heavier one and is
what gets tuned here.

Usage
-----
    python bench/tune_kl_div.py
"""
...
def _reduce_runner(flat_in, flat_tg, block_size, num_warps, num_stages):
    ...
    kernel = _cached_make(
        ntops.kernels.kl_div.reduce_premake,
        log_target=False,
...
def _check_reduce_correctness(dtype):
    """Sanity check that the reduction kernel matches F.kl_div before trusting
    any timing numbers."""
    x = torch.randn(40000, dtype=dtype, device="cuda").log_softmax(dim=-1)
    y = torch.randn(40000, dtype=dtype, device="cuda").softmax(dim=-1)
    run = _reduce_runner(x, y, 1024, 4, 1)
    got = (run() / x.numel()).to(dtype)
    ref = F.kl_div(x, y, reduction="mean")
...
def tune():
    ...
    for dtype in _DTYPES:
        ...
        print(f"\n{'='*92}")
        print(
            f"kl_div config sweep | dtype={dtype} | "
        ...
        for numel in _NUMELS:
            ...
            input = torch.randn(numel, dtype=dtype, device="cuda").log_softmax(dim=-1)
            target = torch.randn(numel, dtype=dtype, device="cuda").softmax(dim=-1)
            output = torch.empty_like(input)

            print(f"\nnumel={numel} (~{side}^2, {numel * itemsize / 1e6:.1f} MB)")

            # Reduction path: reads input + target (2x).
            torch_ms = _time(lambda: F.kl_div(input, target, reduction="sum"))
            _sweep(
                "reduce (sum/mean/batchmean)",
            ...
            torch_ms = _time(lambda: F.kl_div(input, target, reduction="none"))
            _sweep(
                "none (element-wise)",
