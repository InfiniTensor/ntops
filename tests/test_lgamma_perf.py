...
def test_lgamma_perf(shape, dtype):
    a = make_float_input(shape, dtype, op_name="lgamma")
    out = torch.empty_like(a)

    ntops_fn = lambda: ntops.torch.lgamma(a, out=out)
    torch_fn = lambda: torch.lgamma(a, out=out)
    warmup_pair(ntops_fn, torch_fn)

    ntops_us = bench_us(ntops_fn)
    torch_us = bench_us(torch_fn)
    report_and_assert("lgamma", shape, dtype, ntops_us, torch_us)
