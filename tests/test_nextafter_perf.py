...
def test_nextafter_perf(shape, dtype):
    a = make_float_input(shape, dtype)
    b = make_float_input(shape, dtype)
    out = torch.empty_like(a)

    ntops_fn = lambda: ntops.torch.nextafter(a, b, out=out)
    torch_fn = lambda: torch.nextafter(a, b, out=out)
    warmup_pair(ntops_fn, torch_fn)

    ntops_us = bench_us(ntops_fn)
    torch_us = bench_us(torch_fn)
    report_and_assert("nextafter", shape, dtype, ntops_us, torch_us)
