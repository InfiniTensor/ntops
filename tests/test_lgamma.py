...
def test_lgamma_special_values():
    input = torch.tensor(
        [-10.0, -3.0, -2.5, -2.0, -1.5, -1.0, -0.0, 0.0, 1e-6, 0.5, 1.0, 2.0, float("inf"), float("nan")],
        dtype=torch.float32,
        device="cuda",
    )
...
def test_lgamma_scalar():
    input = torch.tensor(2.5, dtype=torch.float32, device="cuda")
...
def test_lgamma_resizes_out_like_torch():
    input = torch.rand((2, 3), dtype=torch.float32, device="cuda") + 0.25
