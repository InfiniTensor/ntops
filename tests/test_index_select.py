...
@skip_if_cuda_not_available
def test_index_select_single_index():
    """Test index_select with a single index (float32)"""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    shape = (3, 4, 5)
    input = torch.randn(shape, dtype=torch.float32, device=device)

    # Test with single index
    indices = torch.tensor([1], device=device, dtype=torch.int64)
    ...
    for dim in range(len(shape)):
        ...
        assert torch.allclose(ninetoothed_output, reference_output), \
            f"Mismatch with single index for dim={dim}"
